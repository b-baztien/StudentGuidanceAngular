...
export class MajorService {
  ...
  getMajorByFacultyReference(facultyRef: DocumentReference) {
    return this.firestore
      .collection(facultyRef.parent.path)
      .doc(facultyRef.id)
      .collection("Major", (query) => query.orderBy("majorName"))
      .snapshotChanges()
  ...
  async addMajor(facultyRef: DocumentReference, major: Major) {
    try {
      await this.firestore
        ...
        .doc(facultyRef.id)
        .collection("Major", (query) =>
          query.where("majorName", "==", major.majorName)
        )
        .get()
        .toPromise()
        .then(async (result) => {
  ...
  async updateMajor(majorRef: DocumentReference, major: Major) {
    try {
      this.careerService.addAllCareer(
        major.listCareerName.map((career_name) => {
