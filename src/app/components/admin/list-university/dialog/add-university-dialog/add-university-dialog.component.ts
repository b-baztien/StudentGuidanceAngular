...
export class AddUniversityDialogComponent implements OnInit, ErrorStateMatcher {
  ...
  universityAddressForm = new FormGroup({
    ...
    tambon: new FormControl(null, [Validators.required]),
    amphur: new FormControl(null, [Validators.required]),
  ...
  async upload(file: File, filePath: string, filename?: string) {
    ...
    if (file.type.includes("image")) {
      return await this.afStorage
        .upload(`${filePath}/${newFileName}`, file, metadata)
        .then(async (result) => {
  ...
  async onSubmit() {
    ...
    try {
      ...
      new Notifications().showNotification(
        ...
        "top",
        "right",
        "เพิ่มข้อมูลมหาวิทยาลัยสำเร็จแล้ว",
        "success",
        "สำเร็จ !"
      );
