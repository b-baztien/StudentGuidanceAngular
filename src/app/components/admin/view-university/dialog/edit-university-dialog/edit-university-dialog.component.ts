...
export class EditUniversityDialogComponent implements OnInit {
  ...
  @ViewChild("highlightInput", { static: false }) highlightInput: ElementRef<
    HTMLInputElement
  >;
  ...
  constructor(
    ...
    public data: { universityId: string; university: University }
  ) {
  ...
  async onSubmit() {
    try {
      if (this.universityDetailForm.valid) {
        ...
        this.university.address = this.universityAddressForm.get(
          "address"
        ).value;
        ...
        this.university.province = this.universityAddressForm.get(
          "province"
        ).value;
        this.university.zipcode = this.universityAddressForm.get(
          "zipcode"
        ).value;
        this.university.zone = this.universityAddressForm.get(
          "province"
        ).value.zone;
        this.university.url = this.universityDetailForm.get("url").value;
        this.university.phone_no = this.universityDetailForm.get(
          "phone_no"
        ).value;
        ...
        this.university.albumImage =
          this.university.albumImage === undefined
            ? new Array<string>()
            : this.university.albumImage;
        ...
        const universityId = await this.universityService.updateUniversity(
          this.data.universityId,
          this.university
        );
        new Notifications().showNotification(
          ...
          "top",
          "right",
          "แก้ไขข้อมูลมหาวิทยาลัยสำเร็จแล้ว",
          "success",
          "สำเร็จ !"
        );
