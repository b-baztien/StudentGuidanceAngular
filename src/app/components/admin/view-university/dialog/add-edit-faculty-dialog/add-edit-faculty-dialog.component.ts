...
export class AddEditFacultyDialogComponent
  ...
  facultyForm = new FormGroup({
    ...
    facultyIcon: new FormControl(
      this.data === null
        ? null
        : this.icons.filter(
            (icon) => icon.name === this.data.facultyIcon.name
          )[0].name,
      [Validators.required]
    ),
  });
  ...
  async onSubmit() {
    ...
    faculty.facultyIcon = this.icons.filter(
      (icon) => icon.name === this.facultyForm.controls.facultyIcon.value
    )[0];
